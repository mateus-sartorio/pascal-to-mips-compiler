...
# Compile the CLI compiler. src/webapp needs Spring, which Maven resolves, so it is skipped
javac:
	@mkdir -p $(JAVA_BIN_DIR)
	$(JAVAC) -cp "$(SRC_DIR):$(ANTLR_PATH)" -d $(JAVA_BIN_DIR) $$(find $(SRC_DIR) -name "*.java" -not -path "$(SRC_DIR)/webapp/*")
